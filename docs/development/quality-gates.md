...
- Generic payment, logistics, and evidence idempotency remains process-local and test-only. Issue #13 adds account-bound durable PostgreSQL replay only for the pure database `order.submit.v1` command. No generalized provider idempotency, callback authentication, durable inbox, worker, ledger, accepted-order mutation, or complete release/refund/settlement reconciliation exists.
